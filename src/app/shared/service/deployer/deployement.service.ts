      ...
      .subscribe(
        applications => {
          ...
          for (let i = 0; i < applications.length; i++) {
            ...
            if (applications[i]['repoUri'] === deployment.configurationParameters.getApplication().phenomenal_version.url) {
    ...
    this._deploymentService.getAll(
      ...
      deployments => {
        ...
        for (let i = 0; i < deployments.length; i++) {
          if (deployments[i]['repoUri'] === deployment.configurationParameters.getApplication().phenomenal_version.url) {
            dep = deployments[i];
